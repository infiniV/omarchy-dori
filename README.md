...
Everything the panel does, the three scripts in [`bin/`](bin) also do from a
...
| Panel | everything, plus codec, bitrate, and the blank-screen toggle |
...
```bash
bin/dori-status                 # one JSON blob: device, camera, mirror
...
Dori is a QML panel plus four bash scripts. It starts `scrcpy` and reads `adb`,
...
- Nothing is downloaded and executed. Nothing is sent anywhere. The phone talks
  to this machine over the cable and stops there.
